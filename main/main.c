...
bleprph_advertise(void)
{
    ...
    rc = ble_gap_adv_start(own_addr_type, NULL, BLE_HS_FOREVER,
                           &adv_params, bleprph_gap_event, NULL);
    if (rc == BLE_HS_EBUSY) {
        // Already advertising or connecting
        ESP_LOGW(TAG, "Advertisement already running");
    } else if (rc != 0) {
        ESP_LOGE(TAG, "Failed to start advertising; rc=%d", rc);
    } else {
        ESP_LOGI(TAG, "Advertising started (multiple connections supported)");
    }
}
...
//         /* Clear per-connection auth state */
//         ESP_LOGI(TAG, "Clearing auth for conn_handle=%u", event->disconnect.conn.conn_handle);
//         biz_clear_auth(event->disconnect.conn.conn_handle);
...
bleprph_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_CONNECT:
        ...
        if (event->connect.status == 0) {
            ...
            bleprph_advertise();
            //init security for the connection
            /* NEW: generate nonce & notify client */
            sec_on_connect(event->connect.conn_handle);
    ...
    case BLE_GAP_EVENT_DISCONNECT:
        ...
        // Clear both security and business logic auth state
        sec_clear_conn(event->disconnect.conn.conn_handle);
        biz_clear_auth(event->disconnect.conn.conn_handle);

        // Resume advertising for new peers
        bleprph_advertise();
        return 0;
    ...
    case BLE_GAP_EVENT_ENC_CHANGE:
        rc = ble_gap_conn_find(event->enc_change.conn_handle, &desc);
        assert(rc == 0);
        ESP_LOGI(TAG, "Encryption state changed: encrypted=%d, bonded=%d",
                 desc.sec_state.encrypted, desc.sec_state.bonded);
        return 0;

    case BLE_GAP_EVENT_REPEAT_PAIRING:
        // Remove the old bond and retry
        rc = ble_gap_conn_find(event->repeat_pairing.conn_handle, &desc);
        assert(rc == 0);
        ble_store_util_delete_peer(&desc.peer_id_addr);
    ...
    case BLE_GAP_EVENT_SUBSCRIBE:
        ESP_LOGI(TAG, "Subscribe event: conn_handle=%d, attr=%d",
...
void app_main(void)
{
    ...
    rc = ble_svc_gap_device_name_set("Airshifter-BLE");
    assert(rc == 0);
    ...
    ESP_LOGI(TAG2, "[APP] Free memory: %" PRIu32 " bytes", esp_get_free_heap_size());
    ...
    // keep commented out to avoid conflicts with wifi_manager and nvs inited earlier
